...
Output is always written next to the input file, named `_output_YYYYMMDD_HHMMSS` with the same extension as the input.
...
fftools convert input.mp4 --resolution 720p

# Explicit dimensions
fftools convert input.mp4 --resolution 1280x720
```
...
fftools convert input.mkv --video-codec h265
```
...
fftools convert input.mkv --audio-codec opus --audio-bitrate 192k
...
```bash
fftools convert input.mkv --resolution 1080p --quality medium --framerate 30
```

### Preview the command without running it

```bash
fftools convert input.mkv --resolution 720p --quality low --dry-run
```
...
```
fftools convert INPUT_FILE [OPTIONS]
